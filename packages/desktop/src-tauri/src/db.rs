//! Database initialization and migration for Double Bind SQLite.
//!
//! Handles first-launch schema creation and PRAGMA configuration.
//! The schema DDL is embedded from packages/migrations/src/sqlite/001-initial-schema.ts.

use rusqlite::Connection;

/// Full DDL for the initial schema (version 1).
/// Source: packages/migrations/src/sqlite/001-initial-schema.ts
...
/// Apply PRAGMA settings for optimal SQLite performance.
fn apply_pragmas(conn: &Connection) -> Result<(), String> {
    ...
    .map_err(|e| format!("Failed to apply PRAGMA settings: {}", e))
}

/// Get the current schema version from the database.
/// Returns 0 if the schema_metadata table does not exist (new database).
...
/// Initialize the database: open/create the SQLite file, apply PRAGMAs,
/// and run migrations if needed.
pub fn init_database(path: &str) -> Result<Connection, String> {
    ...
    if version == 0 {
        conn.execute_batch(SCHEMA_V1)
            .map_err(|e| format!("Failed to apply initial schema: {}", e))?;
    }
