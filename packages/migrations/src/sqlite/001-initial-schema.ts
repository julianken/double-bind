// 001-initial-schema: Creates all core tables for Double-Bind SQLite
//
// This migration establishes the complete SQLite schema with corrections
// identified during Phase 2 architecture review:
//
...
//
// Creates:
// - Primary data: pages, blocks
// - References: block_refs, links
// - Typed metadata: block_properties, page_properties, block_tags, page_tags
// - History: block_history
// - Lookups: daily_notes, schema_metadata, saved_queries
// - FTS5: blocks_fts, pages_fts, saved_queries_fts (contentless)
// - Triggers: FTS sync, auto-update timestamps

import type { SqliteMigration } from '../sqlite-types.js';

/**
 * Initial schema migration for Double-Bind SQLite.
 *
 * Implements production-ready schema with all Phase 2 review corrections applied.
 */
...
-- Drop triggers first
DROP TRIGGER IF EXISTS saved_queries_updated_at;
DROP TRIGGER IF EXISTS blocks_updated_at;
DROP TRIGGER IF EXISTS pages_updated_at;
DROP TRIGGER IF EXISTS saved_queries_fts_delete;
DROP TRIGGER IF EXISTS saved_queries_fts_update;
DROP TRIGGER IF EXISTS saved_queries_fts_insert;
...
-- Drop FTS5 tables
DROP TABLE IF EXISTS saved_queries_fts;
...
-- Drop typed properties and tags
...
-- Drop primary tables (blocks before pages due to FK)
