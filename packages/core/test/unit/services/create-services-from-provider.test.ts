 ...
 * 1. Calls provider.getDatabase() to obtain GraphDB
 * 2. Passes the GraphDB to createServices()
 * 3. Returns a complete Services object
 */

import { describe, it, expect, vi } from 'vitest';
import { MockGraphDB } from '@double-bind/test-utils';
import type { GraphDBProvider } from '@double-bind/types';
...
class MockGraphDBProvider implements GraphDBProvider {
  private db: MockGraphDB;
  private getDatabaseCallCount = 0;

  constructor(db?: MockGraphDB) {
    this.db = db ?? new MockGraphDB();
  }

  async getDatabase(): Promise<MockGraphDB> {
...
describe('createServicesFromProvider', () => {
  describe('factory behavior', () => {
    it('should call provider.getDatabase() exactly once', async () => {
      const provider = new MockGraphDBProvider();
    ...
    it('should use the database from provider for services', async () => {
      ...
      db.seed('pages', []);
      const provider = new MockGraphDBProvider(db);
